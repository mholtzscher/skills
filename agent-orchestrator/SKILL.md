...
Ask for concise conclusions, file and symbol references, commands run, test results, and blockers—not entire files or raw logs when a summary suffices.
...
For example, prepare separate backend-explorer, frontend-explorer, and API-researcher contracts, then batch two `Explore` calls and one `general-purpose` call, all in the foreground. Inspect all three results before starting dependent implementation.
...
3. Inspect and integrate implementation results.
4. Have a tester validate the change, including the original reproduction path for bugs.
...
- delegated changes and evidence were integrated, conflicting findings resolved, and material review findings addressed
