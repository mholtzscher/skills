---
name: visual-explainer-tailwind
description: Generate beautiful, self-contained HTML pages that visually explain systems, code changes, plans, and data using Tailwind CSS and daisyUI. Use when the user asks for a diagram, architecture overview, diff review, plan review, project recap, comparison table, or any visual explanation of technical concepts. Also use proactively when you are about to render a complex ASCII table (4+ rows or 3+ columns) — present it as a styled HTML page instead.
license: MIT
...
Detailed prompt templates live in `./commands/`. Keep the skill harness-agnostic: use these files as reusable command/workflow references, and map them into whatever command system your agent harness supports.
...
**For Tailwind/daisyUI layout patterns, component composition, and SVG connectors**, read `./references/css-patterns.md`.

**Base stack:** Default to the no-build CDN stack unless the user explicitly asks for something else:
...
Use daisyUI primitives for common UI first: `card`, `stats`, `badge`, `table`, `collapse`, `menu`, `navbar`, `steps`, `timeline`, `mockup-window`, `hero`. Use light custom CSS only when Tailwind utilities and daisyUI components cannot express the layout cleanly.
...
Every diagram is a single self-contained `.html` file. No external assets except CDN links (fonts, Tailwind/daisyUI, optional libraries). Structure:
...
<body>
  <!-- Semantic HTML with Tailwind utilities and daisyUI components -->
...
**Requirements:**
- vercel-deploy helper available via `VERCEL_DEPLOY_SCRIPT` or a standard sibling skills directory
